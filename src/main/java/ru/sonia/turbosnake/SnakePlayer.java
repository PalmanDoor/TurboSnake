...
public class SnakePlayer {
    ...
    private final TurboSnake.GameState gameState;
    ...
    public SnakePlayer(String name, Vector3f startPos, float startAngle, Material mat, Node parent, AssetManager am, Node guiNode, Camera cam, PhysicsSpace space, TurboSnake.GameState gameState) {
    ...
    public void update(float tpf, float maxSpeed, float turnSpeed, float spacing) {
        ...
        if (iceMode) {
            ...
            float iceMaxSpeed = TurboSnake.GameState.SPEED * 1.8f;
            ...
            if (turnLeft || turnRight) {
                float turnAngle = (turnLeft ? TurboSnake.GameState.TURN_SPEED : -TurboSnake.GameState.TURN_SPEED) * tpf * 0.6f; // очень медленный поворот
            ...
            for (int i = 1; i < segPos.size(); i++) {
                ...
                if (dist > TurboSnake.GameState.SEG_SPACING) {
                    cur.addLocal(diff.normalize().mult(dist - TurboSnake.GameState.SEG_SPACING));
    ...
    private void addSegment(Vector3f pos) {
        ...
        if (physicsSpace!=null && segments.size()>0) {
            RigidBodyControl phy = new RigidBodyControl(new SphereCollisionShape(SEG_R), 1f);
    ...
    public void attachCactusFragment(Node fragNode, Vector3f worldPos) {
        ...
        cactusAttachments.add(new CactusAttachment(fragNode, segIdx, along, side, up, localRot, 8.0f));
    }
    ...
    public void triggerDeath(Node scene) {
        ...
        for (Geometry seg:segments) {
            RigidBodyControl phy=seg.getControl(RigidBodyControl.class);
